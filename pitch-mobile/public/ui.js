...
export const renderHand = (player, handEl, selectedDiscards, handleCardClick, createCardElement, isPlayer) => {
    if (!handEl) return;
    handEl.innerHTML = '';
...
export const updateUI = (gameData, selectedDiscards, handleCardClick, createCardElement, handleBid, currentUser) => {
    if (!gameData || !gameData.players) return;

    const playerIndex = gameData.players.findIndex(p => p.id === currentUser.uid);
    const reorderedPlayers = [...gameData.players.slice(playerIndex), ...gameData.players.slice(0, playerIndex)];

    renderWidow(gameData, createCardElement);
    reorderedPlayers.forEach((p, i) => renderHand(p, playerHandElements[i], selectedDiscards, handleCardClick, createCardElement, p.id === currentUser.uid));
